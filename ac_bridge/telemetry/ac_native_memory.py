"""
Native Assetto Corsa shared memory reader.

Based on: https://github.com/JanBalke420/DIY-DirectDrive/blob/master/AssettoCorsaMemoryMapping.py

This reads AC's shared memory directly without external libraries.
Much more accurate for original AC (not ACC).
"""
import mmap
import ctypes
...
class SPageFilePhysics(ctypes.Structure):
    """Physics data structure - updated every physics tick."""
    _pack_ = 4
    _fields_ = [
        ('packetId', c_int32),
        ('gas', c_float),
        ...
        ('cgHeight', c_float),
        ('carDamage', c_float * 5),
        ('numberOfTyresOut', c_int32),  # KEY: How many tires off track!
...
class SPageFileGraphic(ctypes.Structure):
    """Graphics/timing data - updated every frame."""
    _pack_ = 4
    _fields_ = [
        ('packetId', c_int32),
        ('status', AC_STATUS),
        ...
        ('idealLineOn', c_int32),
        ('isInPitLane', c_int32),
        ('surfaceGrip', c_float),  # Track surface grip
...
class SPageFileStatic(ctypes.Structure):
    """Static session info - set at session start."""
    _pack_ = 4
    _fields_ = [
...
class ACSharedMemory:
    ...
    Reads directly from AC's memory-mapped files.
    """
    ...
    def __init__(self):
        ...
        try:
            ...
            logger.info("ac_shared_memory_initialized")
        except Exception as e:
            logger.error("failed_to_open_shared_memory", error=str(e))
            raise RuntimeError(
    ...
    def is_connected(self) -> bool:
        """Check if AC is running and sending data."""
        try:
            return self.graphics.status in [AC_LIVE, AC_PAUSE, AC_REPLAY]
        except:
            return False
